...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [OldHitRanges version `$version` is out!](https://modrinth.com/mod/oldhitranges/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [OldHitRanges version `$version` is out!](https://modrinth.com/mod/oldhitranges-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
